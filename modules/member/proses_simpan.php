...
if (isset($_POST['simpan'])) {
    ...
    if (move_uploaded_file($tmp_file, $path)) {
        ...
        $insert = mysqli_query($mysqli, "INSERT INTO tbl_member(id_member, tanggal_gabung, jabatan, nama_lengkap, jenis_kelamin, alamat, email, whatsapp, foto_profil) 
                                         VALUES('$id_member', '$tanggal_gabung', '$jabatan', '$nama_lengkap', '$jenis_kelamin', '$alamat', '$email', '$whatsapp', '$nama_file_enkripsi')")
                                         or die('Ada kesalahan pada query insert : ' . mysqli_error($mysqli));
        // cek query
        // jika proses insert berhasil
        if ($insert) {
            // alihkan ke halaman data member dan tampilkan pesan berhasil simpan data
            header('location: ../../main.php?module=member&pesan=1');
        }
    }
}
