...
if (isset($_POST['simpan'])) {
    ...
    if (empty($nama_file)) {
        // sql statement untuk update data di tabel "tbl_member" berdasarkan "id_member"
        $update = mysqli_query($mysqli, "UPDATE tbl_member
                                         SET tanggal_gabung='$tanggal_gabung', jabatan='$jabatan', nama_lengkap='$nama_lengkap', jenis_kelamin='$jenis_kelamin', alamat='$alamat', email='$email', whatsapp='$whatsapp'
                                         WHERE id_member='$id_member'")
                                         or die('Ada kesalahan pada query update : ' . mysqli_error($mysqli));
        // cek query
        // jika proses update berhasil
        if ($update) {
            // alihkan ke halaman detail data member dan tampilkan pesan berhasil ubah data
            header("location: ../../main.php?module=tampil_detail_member&id=$id_member&pesan=1");
        }
    }
    // jika data foto ada (foto diubah)
        ...
        if (move_uploaded_file($tmp_file, $path)) {
            // sql statement untuk update data di tabel "tbl_member" berdasarkan "id_member"
            $update = mysqli_query($mysqli, "UPDATE tbl_member
                                             SET tanggal_gabung='$tanggal_gabung', jabatan='$jabatan', nama_lengkap='$nama_lengkap', jenis_kelamin='$jenis_kelamin', alamat='$alamat', email='$email', whatsapp='$whatsapp', foto_profil='$nama_file_enkripsi'
                                             WHERE id_member='$id_member'")
                                             or die('Ada kesalahan pada query update : ' . mysqli_error($mysqli));
            // cek query
            // jika proses update berhasil
