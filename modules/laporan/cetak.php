...
$html = '<!DOCTYPE html>
        <html>
        <head>
          	<title>Laporan Data Pegawai Perhutani KPH Bojonegoro</title>
          	<link rel="stylesheet" href="../../assets/css/laporan.css">
        </head>
        <body>
			<div class="text-center">
				<h1>LAPORAN DATA PEGAWAI PERHUTANI KPH BOJONEGORO</h1>
...
if ($rows <> 0) {
	...
	while ($data = mysqli_fetch_assoc($query)) {
		...
		$html .= '		<tr>
							...
							<td width="60" align="center">' . $data['jabatan'] . '</td>
...
$html .= '    		</tbody>
          	...
          	<div class="text-right mt-5">Bojonegoro, ' . tanggal_indo(date('Y-m-d')) . '</div>
        </body>
        </html>';
...
$dompdf->stream('Laporan-Data-Pegawai-Tanggal-' . $tanggal_awal . '-sd-' . $tanggal_akhir . '.pdf', array('Attachment' => 0));
